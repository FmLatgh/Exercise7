  ...
  function truncate(text, n = 200){ if (!text) return ""; return text.length > n ? text.slice(0,n) + "…" : text; }
  function escapeHtml(s){ if (!s) return ""; return s.replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); }
  function safeName(n){ return n?.name || n?.nutrientName || n?.nutrient || "Unknown"; }
  function normalizeNutrient(n){ // ensure { name, amount, unitName, number }
    return {
      name: safeName(n),
  ...
  async function fetchUSDA(q, apiKey){
    ...
    return data.map(it => ({
      ...
      foodNutrients: (Array.isArray(it.foodNutrients) ? it.foodNutrients.map(normalizeNutrient) : []),
      raw: it
    }));
  }
  ...
  async function fetchOFF(q){
    ...
    const url = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(q)}&search_simple=1&action=process&json=1&page_size=50`;
    ...
    return products.map(p => {
      ...
      if(p.nutriments){
        ...
        for(const [k,v] of Object.entries(p.nutriments)){
          ...
          nutr.push({ name, amount: v, unitName: p.nutriments[`${k}_unit`] ?? "" , number: null });
        }
      }
      return {
        ...
        code: p.code,
        foodNutrients: nutr.map(n => ({ name: n.name, amount: n.amount, unitName: n.unitName, number: null })),
        raw: p
      };
    });
  }
  ...
  async function fetchCNF(q){
    ...
    const url = `https://food-nutrition.canada.ca/api/canadian-nutrient-file/foods?search=${encodeURIComponent(q)}&page_size=50`;
    const resp = await fetch(url);
    ...
    return json.map(it => ({
      ...
      source: "CNF",
      foodNutrients: (Array.isArray(it.nutrients) ? it.nutrients.map(n => ({
  ...
  async function fetchAll(q, apiKey){
    ...
    const promises = [
      (async ()=> { try { return await fetchOFF(q); } catch(e){ console.warn("OFF failed", e); return []; }})(),
      (async ()=> { try { return await fetchCNF(q); } catch(e){ console.warn("CNF failed", e); return []; }})()
    ];
    if(apiKey){
      promises.push((async ()=> { try { return await fetchUSDA(q, apiKey); } catch(e){ console.warn("USDA failed", e); return []; }})());
    } else {
  ...
  function renderItems(items, limit){
    ...
    sliced.forEach((item, idx) => {
      ...
      idSpan.textContent = item.fdcId ? `fdcId: ${item.fdcId}` : (item.code ? `code: ${item.code}` : "");
      ...
      desc.innerHTML = `<strong>Description:</strong> ${escapeHtml(item.description || "")}`;
      ...
      // nutrients block
      const nutrients = Array.isArray(item.foodNutrients) ? item.foodNutrients.map(n => {
        ...
        if(typeof n === "object" && ("name" in n)) {
          ...
          return { name: String(n), amount: null, unitName: "", number: null };
        }
      }) : [];
      ...
      copyBtn.onclick = () => {
        navigator.clipboard.writeText(JSON.stringify(item.raw ?? item, null, 2)).then(()=> {
  ...
  searchBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      metaEl.textContent = `Source: ${source.toUpperCase()} — Found ${Array.isArray(items)?items.length:0} result(s). Query took ${took}ms.`;
